...
IP_CURRENT=`ifconfig eth0 | awk '/inet addr/{print substr($2,6)}'`
sed -r -i "s/#(advertised.host.name)=(.*)/\1=$IP_CURRENT/g" $KAFKA_HOME/config/server.properties
#sed -r -i "s/(zookeeper.connect)=(.*)/\1=$IP_CURRENT/g" $KAFKA_HOME/config/server.properties
